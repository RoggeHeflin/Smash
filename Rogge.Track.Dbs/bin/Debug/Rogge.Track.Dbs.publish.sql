...
GO
PRINT N'Dropping Extended Property [PRIMARY].[MS_Description]...';
...
GO
ALTER TABLE [verf].[ItemCount] WITH NOCHECK
...
SELECT
		...
		[Duration]					= STUFF(CONVERT(VARCHAR(20),	CONVERT(DATETIME, MAX([t].[EventEnd]))
											- CONVERT(DATETIME, MIN([t].[EventBeg])), 114), 1, 2,
											DATEDIFF(HOUR, 0,		CONVERT(DATETIME, MAX([t].[EventEnd]))
											- CONVERT(DATETIME, MIN([t].[EventBeg])))),
...
ALTER TABLE [verf].[ItemCount] WITH CHECK CHECK CONSTRAINT [CL_ItemCount_ServerName];

ALTER TABLE [verf].[ItemCount] WITH CHECK CHECK CONSTRAINT [CL_ItemCount_DatabaseName];

ALTER TABLE [verf].[ItemCount] WITH CHECK CHECK CONSTRAINT [CL_ItemCount_SchemaName];

ALTER TABLE [verf].[ItemCount] WITH CHECK CHECK CONSTRAINT [CL_ItemCount_TableName];

ALTER TABLE [verf].[ItemCount] WITH CHECK CHECK CONSTRAINT [CL_ItemCount_DescriptionName];
